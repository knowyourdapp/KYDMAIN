...
const Navbar = ({ toggleLoginModal, userWalletAddress }) => {
    const [nav, setNav] = useState(false);
    const [balance, setBalance] = useState('0.00'); // State for wallet balance
    ...
    // Function to fetch the balance from Solflare
    const fetchBalance = async () => {
        try {
            const response = await fetch(`https://api.solscan.io/account/${userWalletAddress}?limit=10`); // Modify based on your API
            const data = await response.json();
            if (data && data.data) {
                // Assuming balance is in the 'sol' key; modify if necessary
                setBalance(data.data[0]?.sol || '0.00'); 
            }
        } catch (error) {
    ...
    useEffect(() => {
        if (userWalletAddress) {
            fetchBalance(); // Fetch balance when the wallet address is available
        }
    }, [userWalletAddress]);
        ...
        <div className='fixed top-0 left-0 w-full blue text-white z-50'>
            <div className='flex md:justify-between items-center md:px-10 h-16 mx-auto px-4'>
                ...
                <div className='ml-4 space-x-2 md:block hidden'>
                    <button className="px-4 py-2 rounded-full bg-#010D1B hover:bg-#010D1B-600 text-xs font-bold transition-colors text-white">
                        ${balance} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
                    </button>
                </div>
            </div>
